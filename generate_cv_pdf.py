...
class ATSNumberedCanvasES(canvas.Canvas):
    ...
    def draw_page_elements(self, page_count):
        ...
        self.setFont("Helvetica", 8)
        self.setFillColor(TEXT_MUTED)
        self.drawString(36, 18, "rubendavid1809@gmail.com • Tel. +54 9 11 7025-9429 • Buenos Aires, Argentina")
...
class ATSNumberedCanvasEN(canvas.Canvas):
    ...
    def draw_page_elements(self, page_count):
        ...
        self.setFont("Helvetica", 8)
        self.setFillColor(TEXT_MUTED)
        self.drawString(36, 18, "rubendavid1809@gmail.com • Phone: +54 9 11 7025-9429 • Buenos Aires, Argentina")
...
def get_styles():
    return {
        ...
        "SectionTitle": ParagraphStyle("SectionTitle", fontName="Helvetica-Bold", fontSize=10.5, leading=12, textColor=PRIMARY, spaceBefore=5, spaceAfter=2, keepWithNext=True),
        "JobTitleLeft": ParagraphStyle("JobTitleLeft", fontName="Helvetica-Bold", fontSize=9.5, leading=11.5, textColor=TEXT_DARK),
        "JobDateRight": ParagraphStyle("JobDateRight", fontName="Helvetica-Oblique", fontSize=8.5, leading=11.5, textColor=TEXT_MUTED, alignment=2),
        "JobRole": ParagraphStyle("JobRole", fontName="Helvetica-Bold", fontSize=9, leading=11, textColor=SECONDARY, spaceAfter=2),
        "Body": ParagraphStyle("Body", fontName="Helvetica", fontSize=8.5, leading=11, textColor=TEXT_DARK, alignment=4),
        "Bullet": ParagraphStyle("Bullet", fontName="Helvetica", fontSize=8.3, leading=10.8, textColor=TEXT_DARK, leftIndent=12, firstLineIndent=-12, spaceAfter=1.5),
        "Achievement": ParagraphStyle("Achievement", fontName="Helvetica-Bold", fontSize=8.3, leading=10.8, textColor=PRIMARY, leftIndent=12, firstLineIndent=-12, spaceAfter=2),
        ...
        "ProjectItem": ParagraphStyle("ProjectItem", fontName="Helvetica", fontSize=8.3, leading=10.8, textColor=TEXT_DARK, leftIndent=10, firstLineIndent=-10, spaceAfter=2.5)
    }

def hr():
    return HRFlowable(width="100%", thickness=0.8, color=PRIMARY, spaceBefore=2, spaceAfter=4)


def build_pdf_es(filename="CV_Ruben_Barrios_Analista_De_Datos.pdf"):
    margin = 36
    doc = SimpleDocTemplate(
        filename, pagesize=letter, leftMargin=margin, rightMargin=margin, topMargin=36, bottomMargin=36,
    ...
    story.append(Paragraph('LinkedIn: <a href="https://linkedin.com/in/ruben-barrios-1430712ab" color="#2B6CB0"><b>linkedin.com/in/ruben-barrios-1430712ab</b></a> &nbsp;|&nbsp; GitHub: <a href="https://github.com/rubenbarrios-bigdata" color="#2B6CB0"><b>github.com/rubenbarrios-bigdata</b></a> &nbsp;|&nbsp; Portafolio: <a href="https://rubenbarrios-bigdata.github.io/cv-ejecutivo-inteligente/" color="#2B6CB0"><b>cv-ejecutivo-inteligente</b></a>', styles["Links"]))

    story.append(Paragraph("RESUMEN PROFESIONAL", styles["SectionTitle"]))
    story.append(hr())
    story.append(Paragraph("<b>Analista de Datos</b> con experiencia demostrada en análisis de métricas de fraude, logística y e-commerce, y más de 15 años de sólida trayectoria previa en el sector bancario y financiero internacional. Especializado en transformar requerimientos de negocio en soluciones analíticas de alto impacto mediante el diseño de dashboards en <b>Power BI (DAX, modelado estrella)</b>, consultas avanzadas en <b>SQL (MySQL, PostgreSQL)</b>, automatización con <b>Python (pandas)</b> y analítica digital de productos con <b>GTM, GA4, Supabase y Looker Studio</b>.", styles["Body"]))
    ...
    story.append(hr())
    skills_data = [
        [Paragraph("<b>Análisis de Datos y BI:</b>", styles["SkillCategory"]), Paragraph("Power BI (DAX avanzado, Power Query, modelado estrella, PBIP), SQL (MySQL, PostgreSQL: queries complejas, subconsultas, window functions), Python (pandas, NumPy, Matplotlib, Colab, VSCode), Excel avanzado (Power Pivot, macros).", styles["SkillText"])],
        [Paragraph("<b>Visualización y Producto:</b>", styles["SkillCategory"]), Paragraph("Looker Studio, Google Analytics 4 (GA4), Google Tag Manager (GTM), Supabase, Firebase, BigQuery, diseño de KPIs ejecutivos, análisis de retención, engagement, funnel y churn.", styles["SkillText"])],
    ...
    story.append(t_p3)
    story.append(Paragraph("Especialista de Comercio Exterior / Analista Financiero y Operaciones (BFC, Banco Exterior, BVC)", styles["JobRole"]))
    ...
    projects = [
        ...
        ("<b>Proyecto SQL Orders & E-Commerce Analysis:</b>", "Análisis exploratorio y transaccional sobre datasets relacionales reales empleando subqueries, JOINs complejos, CTEs y funciones de ventana en SQL para segmentación de clientes y análisis de ventas. " + '<a href="https://github.com/rubenbarrios-bigdata/proyecto-sql-orders" color="#2B6CB0"><b>[Ver en GitHub]</b></a>'),
    ...
    story.append(Paragraph("CERTIFICACIONES TÉCNICAS", styles["SectionTitle"]))
    story.append(hr())
    certs = [
        ("Power BI DevOps: PBIP, Git, CI/CD y Microsoft Fabric", "Daxus Latam"),
        ("Analítica Digital: Google Tag Manager, Google Analytics 4 y Data Studio", "Capacitación Especializada"),
        ...
        ("Iniciación a la Programación con Python", "Educación IT"),
...
def build_pdf_en(filename="CV_Ruben_Barrios_Data_Analyst.pdf"):
    ...
    story.append(Paragraph('LinkedIn: <a href="https://linkedin.com/in/ruben-barrios-1430712ab" color="#2B6CB0"><b>linkedin.com/in/ruben-barrios-1430712ab</b></a> &nbsp;|&nbsp; GitHub: <a href="https://github.com/rubenbarrios-bigdata" color="#2B6CB0"><b>github.com/rubenbarrios-bigdata</b></a> &nbsp;|&nbsp; Interactive Web CV: <a href="https://rubenbarrios-bigdata.github.io/cv-ejecutivo-inteligente/" color="#2B6CB0"><b>cv-ejecutivo-inteligente</b></a>', styles["Links"]))

    story.append(Paragraph("PROFESSIONAL SUMMARY", styles["SectionTitle"]))
    story.append(hr())
    story.append(Paragraph("<b>Data Analyst</b> with demonstrated experience analyzing fraud metrics, logistics, and e-commerce operations, backed by over 15 years of solid prior experience in the international banking and financial sector. Specialized in transforming business requirements into high-impact analytical solutions through <b>Power BI dashboards (DAX, star schema modeling)</b>, advanced <b>SQL queries (MySQL, PostgreSQL)</b>, data automation with <b>Python (pandas)</b>, and digital product analytics with <b>GTM, GA4, Supabase, and Looker Studio</b>.", styles["Body"]))
    story.append(Spacer(1, 3))
    ...
    story.append(hr())
    skills_data = [
        [Paragraph("<b>Data Analytics & BI:</b>", styles["SkillCategory"]), Paragraph("Power BI (Advanced DAX, Power Query, star schema, PBIP), SQL (MySQL, PostgreSQL: complex queries, subqueries, window functions), Python (pandas, NumPy, Matplotlib, Colab, VSCode), Advanced Excel (Power Pivot, macros).", styles["SkillText"])],
        [Paragraph("<b>Visualization & Product:</b>", styles["SkillCategory"]), Paragraph("Looker Studio, Google Analytics 4 (GA4), Google Tag Manager (GTM), Supabase, Firebase, BigQuery, executive KPI design, user retention, engagement, funnel and churn analysis.", styles["SkillText"])],
    ...
    t_p3.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'MIDDLE'), ('LEFTPADDING', (0,0), (-1,-1), 0), ('RIGHTPADDING', (0,0), (-1,-1), 0), ('TOPPADDING', (0,0), (-1,-1), 0), ('BOTTOMPADDING', (0,0), (-1,-1), 0)]))
    story.append(t_p3)
    story.append(Paragraph("Foreign Trade Specialist / Financial & Operations Analyst (BFC, Banco Exterior, BVC)", styles["JobRole"]))
    ...
    projects_en = [
        ...
        ("<b>Customer Service Dashboard (Power BI + DAX):</b>", "Interactive dashboard optimizing support operations: tracking ticket volume, first-response time, resolution SLA, and CSAT using star schema modeling and custom DAX measures. " + '<a href="https://github.com/rubenbarrios-bigdata/dashboard-atencion-al-cliente" color="#2B6CB0"><b>[View on GitHub]</b></a>'),
        ...
        ("<b>SQL Orders & E-Commerce Analysis:</b>", "Exploratory and transactional analysis over real relational datasets using subqueries, complex JOINs, CTEs, and window functions in SQL for customer segmentation and profitability. " + '<a href="https://github.com/rubenbarrios-bigdata/proyecto-sql-orders" color="#2B6CB0"><b>[View on GitHub]</b></a>'),
    ...
    story.append(Paragraph("TECHNICAL CERTIFICATIONS", styles["SectionTitle"]))
    story.append(hr())
    certs_en = [
        ("Power BI DevOps: PBIP, Git, CI/CD, and Microsoft Fabric", "Daxus Latam"),
        ("Digital Analytics: Google Tag Manager, Google Analytics 4, and Data Studio", "Specialized Training"),
        ("Data Analysis: Excel, Power BI, SQL, and Python", "Datax"),
        ("Artificial Intelligence Fundamentals", "IBM SkillsBuild"),
        ("Introduction to Programming with Python", "Educación IT"),
